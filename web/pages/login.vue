  ...
  <div class="flex-grow flex items-center justify-center min-h-24">
    <div class="login-element">
      ...
      <form @focusin="inputActive = true" @focusout="inputActive = false" @submit.prevent="handleUser" class="relative">
        <input type="text" v-model="user" class="py-2 input input-bordered w-full input-accent">
        <button type="submit"
...
const handleUser = async (e: Event) => {
  ...
  if (data) {
    ...
    if (authData) {
      availableUser.value.id = authData.record.id;
      availableUser.value.name = authData.record.name;
      availableUser.value.isAuthenticated = true;
      availableUser.value.record = authData.record;
      ...
      const countRecord = await CounterCollection.create(data);

      console.log('countRecord', countRecord)

      availableUser.value.countRecord = countRecord
